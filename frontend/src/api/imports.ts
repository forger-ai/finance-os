...
import type { ImportResult, PreprocessedDocument } from "./types";
...
export function preprocessImportDocument(file: File): Promise<PreprocessedDocument> {
  const formData = new FormData();
  formData.append("file", file);
  return request<PreprocessedDocument>("/api/imports/preprocess-document", {
    method: "POST",
    body: formData,
  });
}
